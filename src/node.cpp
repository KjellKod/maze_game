...
namespace {
  std::map<Path::Direction, std::string> map_directions
  {{Path::Direction::East, "east"},
  ...
  struct IllegalDirectionException: public std::exception
  {
    ...
    explicit IllegalDirectionException(std::string dir): direction(dir){}
...
std::string Path::directionToString() {
 ...
 Path::Direction Path::stringToDirection(const std::string& dir) {
   ...
   for(auto& pair : map_directions) {
     if(pair.second == dir) return pair.first;
   }
...
bool Node::takeObject(const node::Object& object) {
  node::Object o;
  if(!hasObject(object)) return false;
